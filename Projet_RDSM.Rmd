...
## Objectif de l'analyse et Choix des variables
...
data <- data_raw |>
  select(
date_debut, jour, heure, O3, temperature, pression, pression_variation_3h, humidite, point_de_rosee)
```
...
(1) Le boxplot de l’ozone (O₃) montre une médiane autour de 55 µg/m³, centrée dans la boîte, ce qui traduit une distribution équilibrée et homogène. L’absence de valeurs aberrantes indique peu de fluctuations extrêmes, la majorité des concentrations étant comprises entre 30 et 70 µg/m³.
...
barplot(sort(abs(correlations), decreasing = TRUE), las = 2, col = "steelblue", main = "Corrélation absolue avec O3", cex.names = 0.7)
```
\vspace{-2em}
